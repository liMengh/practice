...
class Request {
	...
	request(options = {}) {
		...
		// 需要埋点的接口
		const urlArr = ['/wx/home/index', '/wx/seckill/submit', '/open/order/syncOrder', '/wx/CustomerDistributorl/joinBooking', '/wx/help/bargain/submit', '/wx/order/submit', '/wx/order/balancePrepay', '/wx/home/coupon/listApp', '/wx/seckill/getSeckillGoods', '/wx/goods/detail']
		const urlArr1 = ['/wx/auth/login_by_weixin', '/wx/auth/login']
		function urlCheck(url, urlArr) {
			var some = urlArr.some((value) => url.indexOf(value) > -1)
			return some
		}
		if (urlCheck(options.url, urlArr)) {
			const statistics = store.state.user.statistics
			// console.log("statistics",statistics)
			let activityCode = uni.getStorageSync('activityCode') || ''
			let distributorId = uni.getStorageSync('distributorId') || ''
			var obj = {
				'distributorId': statistics.distributorId ? statistics.distributorId : distributorId,
				'activityCode': statistics.activityCode ? statistics.activityCode : activityCode
				// 'activityCode': statistics.activityCode || statistics.activityId ? statistics.activityCode || statistics.activityId : ''
			}
			options.data = Object.assign(options.data, obj)
		}
		const statistics = store.state.user.statistics
		if (statistics.channelManageId) {
			if (urlCheck(options.url, urlArr1)) {
				var obj = {
					'channelManageId': statistics.channelManageId ? statistics.channelManageId : ''
				}
				options.data = Object.assign(options.data, obj)
			}
		}
	...
	*/
	...
	_success = function(that, _config, res, resolve, reject) {
      ...
      // console.log("result:",result)
			...
			if (skip || result.success || result.errno) { // 接口调用业务成功
